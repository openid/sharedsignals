...
> **Transmitter-Supplied**, A string that uniquely identifies the stream. A
  transmitter MUST generate a unique ID for each of its streams at the time of
  stream creation.
...
> **Transmitter-Supplied**, An array of URIs identifying the set of events that
  the Transmitter SHALL include in the stream. This is a subset (not necessarily
...
* `events_requested` : This MUST NOT be empty
...
request MUST NOT be changed by the Transmitter. If the `events_requested` parameter
is present in the request, its value MUST NOT be an empty array.
